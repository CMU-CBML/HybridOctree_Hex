...
inline void hexGen::GetCellValue() {
	...
	for (i = 0; i < triMesh.eNum; i++) {
		...
		for (j = i + 1; j < triMesh.eNum; j++) {
			...
			// The shipped code scales the ray-cast thickness by the dominant
			// component of the (unit) triangle normal, always in [1/sqrt(3), 1],
			// so it always shrinks the measured distance. Building with
			// -DRAW_THICKNESS uses the true Euclidean distance instead, to test
			// whether the pre-v1.0 code had that factor at all.
#ifdef RAW_THICKNESS
			len = abs(len);
#else
...
inline void hexGen::ComputeCellValue(int octreeId, int level, bool& intersect) {
	// LADDER_TOP is the octree level of the deepest refinement tier (the one
	// gated by C_THRES[4]/H_THRES[4]); the five tiers sit at LADDER_TOP-4 ...
	// LADDER_TOP and "refine all" at LADDER_TOP-5. The shipped default 8
	// reproduces v1.0's original hardcoded ladder byte-for-byte; building with
	// -DLADDER_TOP="(octreeDepth - 1)" anchors it to VOXEL_SIZE instead, which
	// is what lets VOXEL_SIZE act as the paper's per-model refinement-level knob.
	const int ladderTop = LADDER_TOP;
	int i;
	...
	else if (level == ladderTop - 5)// refine all
		for (i = 0; i < triMesh.eNum; i++) {
			for (j = 0; j < 3; j++)
